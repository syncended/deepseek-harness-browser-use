...
describe('BrowserManager', () => {
  it('navigates, snapshots, acts, screenshots, and enforces human control', async () => {
    ...
    try {
      ...
      const observed = await browser.screen('observer')
      expect(observed.controlOwner).toBe('agent')
      ...
      expect(resized.controlOwner).toBe('self')
      expect(resized.width).toBe(1024)
      expect(resized.height).toBe(640)
      const observedWhileHumanControls = await browser.screen('observer')
      expect(observedWhileHumanControls.controlOwner).toBe('other')
